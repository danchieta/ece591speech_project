function [x] = cepstral_analysis(x, win_duration, overlap_duration,...
	...
	[x.spectrum_ofhigh, x.spec_index] = power_spectrum(x.snaps_highenergy, x.fs);

	[x.smooth_spectrum, x.sm_spec_index] = mel_filterbank(x.spectrum_ofhigh,...
		x.spec_index, smooth_nwin)

	[x.cepstrum_coef, x.cepstrum_coef_centr] = mel_cepstrum(x.smooth_spectrum,...
		ncepstrum);
